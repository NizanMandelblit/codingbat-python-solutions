...
def near_hundred(n):
    ...
    """
    hundred = abs(n-100)
    two_hundred = abs(n-200)
    return (hundred <= 10) or (two_hundred <= 10)
...
def not_string(str):
    ...
    """
    if "not" in str[:3]:  # also possible if str.startswith("not")
        return str
    return "not "+str
...
def front_back(str):
    ...
    mid = str[1:-1]  # Everything except the first and last characters
